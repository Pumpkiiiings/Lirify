...
class PacketHandler {
    ...
    async processPacket(session, direction, data, meta) {
        ...
        // Lirify: all packets are interceptable (no isSafe restriction)
        if (session.proxy.pluginAPI.events.hasPacketInterceptors(direction, meta.name)) {
            const result = await this.handleInterceptors(session, direction, data, meta, true);
        ...
        // Lirify: feed server packets into world state reconstruction
        if (direction === 'server' && session.proxy.worldState) {
            try {
                session.proxy.worldState.handleServerPacket(meta.name, data);
            } catch (e) {
                // non-fatal
            }
        }
