class Players {
    ...
    getPlayers() {
        ...
        for (const [entityId, entity] of gameState.entities) {
            if (entity.type === 'player' && entity.uuid) {
                ...
                const name = playerInfo?.name || gameState.uuidToName.get(entity.uuid) || entity.name || 'Unknown Player';
                
                entitiesWithinRender.add(entity.uuid);
                players.push({
                    uuid: entity.uuid,
                    name: name,
                    displayName: gameState.getFormattedName(entity.uuid) || name,
    ...
    getCurrentPlayer() {
        return this.getPlayers().find(p => p.isCurrentPlayer);
    }
    ...
    getPlayerName(uuid) {
        ...
        return this.proxy.currentPlayer.gameState.uuidToName.get(uuid) || null;
    }
