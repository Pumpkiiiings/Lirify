class EntityHandler {
    ...
    handleNamedEntitySpawn(data) {
        const playerInfo = this.gameState.playerInfo.get(data.playerUUID);
        const cachedName = this.gameState.uuidToName.get(data.playerUUID);
        
        const customNameMeta = data.metadata ? data.metadata.find(m => m.key === 2) : null;
        const customName = customNameMeta ? (typeof customNameMeta.value === 'string' ? customNameMeta.value : null) : null;
        
        if (!playerInfo && !cachedName && !customName) {
            console.log(`[DEBUG] Unknown Entity UUID: '${data.playerUUID}'`);
            console.log(`[DEBUG] uuidToName size: ${this.gameState.uuidToName.size}`);
            console.log(`[DEBUG] uuidToName keys sample:`, Array.from(this.gameState.uuidToName.keys()).slice(0, 5));
        }

        const newEntity = {
            type: 'player',
            uuid: data.playerUUID,
            name: playerInfo?.name || cachedName || customName || null,
    ...
    handleEntityMetadata(data) {
        if (this.gameState.entities.has(data.entityId)) {
            ...
            data.metadata.forEach(newMeta => {
                ...
                if (newMeta.key === 2 && typeof newMeta.value === 'string') {
                    if (!entity.name || entity.name === 'Unknown Player') {
                        entity.name = newMeta.value;
                    }
                }
            });
