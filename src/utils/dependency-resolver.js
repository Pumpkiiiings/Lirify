    ...
    _checkDependencyVersion(depPlugin, requirement) {
        try {
            ...
            if (requirement.version) {
                const match = VersionUtils.compare(depPlugin.version, requirement.version) === 0;
                if (!match) {
                    return {
                        compatible: false,
            ...
            if (requirement.minVersion) {
                if (!VersionUtils.satisfies(depPlugin.version, `>=${requirement.minVersion}`)) {
                    return {
                        compatible: false,
            ...
            if (requirement.maxVersion) {
                if (!VersionUtils.satisfies(depPlugin.version, `<=${requirement.maxVersion}`)) {
                    return {
                        compatible: false,
