...
class CommandHandler {
    constructor(proxy) {
        ...
        this.THEME = THEME;
        // Map: aliasName (string) → moduleName (string)
        // Populated automatically when plugins register subcommands
    ...
    register(moduleName, registrationFunction) {
        ...
        // Auto-register top-level aliases for every subcommand
        // so /rq works the same as /requeue rq
        for (const sub of moduleCommand.commands) {
            const subName = sub.name();
            if (['config', 'help'].includes(subName)) continue;
            if (!this.commandAliases.has(subName)) {
                this.commandAliases.set(subName, normalizedModuleName);
            }
        }
    }
    ...
    handleCommand(message, client) {
        ...
        let moduleName = parts[0]?.toLowerCase();
        let args = parts.slice(1);

        // Direct module match (e.g. /proxy server)
        if (!this.modules.has(moduleName)) {
            // Check if it's a top-level alias for a subcommand (e.g. /rq → requeue rq)
            const aliasTarget = this.commandAliases.get(moduleName);
            if (aliasTarget) {
                // Rewrite as /requeue rq [extra args]
                args = [moduleName, ...args];
                moduleName = aliasTarget;
            } else {
                return false;
            }
        }
