...
function createAutoConfig({ commandHandler, moduleName, options, client }) {
    ...
    // Get config directly from the plugin's Core instance
    // The Core stores the live config with get/set/saveCurrentConfig
    const pluginCore = new (require('../plugin-api/core'))(commandHandler.proxy, loadedPlugin.metadata);
    const configObject = pluginCore.config.get() || {};
    
    const configSchema = loadedPlugin.metadata.configSchema;
    
    const schema = buildAutoSchema(moduleName, configSchema);
    
    // Helpers that save through Core
    const saveConfig = () => {
        pluginCore.initializeConfig(schema); // re-merges defaults without overwriting
    };
    
    const pluginWrapper = {
        saveCurrentConfig: () => {
            // Write back configObject via Core's set
            const flatten = (obj, prefix = '') => {
                for (const [k, v] of Object.entries(obj)) {
                    ...
                    if (v && typeof v === 'object' && !Array.isArray(v)) {
                        ...
                        pluginCore.config.set(key, v);
                    }
                }
            };
            flatten(configObject);
        }
    };
